...
//	use share memory
	__shared__ float4 tileB[32][8]; //bk*bn
	...
	for(tileNum=0 ; tileNum < totTileNum; tileNum++){
		...
		int index = (blockz*KtileWidth + tileNum*bk + idMod32)<<2;
		if(index <= k){
            		tileB[idMod32][idDivide32] = reinterpret_cast<float4*> (B)[idDivide32 * k + index>>2];
			//printf("id=%d,x=%f,y=%f,z=%f,w=%f,B[%d][%d]\n",id,tileB[idMod32][idDivide32].x,tileB[idMod32][idDivide32].y,tileB[idMod32][idDivide32].z,tileB[idMod32][idDivide32].w,idDivide32,index);
        	}
		for(currentRow=0; currentRow < bm; currentRow++){
			...
			//for(loop=0; (loop*subtileN)<bn; loop++){
				if(index <= k){
					int offset =( ((blocky*bm + currentRow)*k + blockz*KtileWidth + tileNum*bk) >> 2 )+ idMod32;
					float4 a = tex1Dfetch(texture_A, offset);
					float4 b = tileB[idMod32][idDivide32];
			...
			*/
        	    ...
        	    if(idMod32 == 0) {
					...
					tempC[currentRow][idDivide32] +=value;		
					...
					if(tileNum == totTileNum-1){
						...
						atomicAdd(&_C[ (currentRow+blockx*bm)*n + blocky*bn + idDivide32] , tempC[currentRow][idDivide32]);
